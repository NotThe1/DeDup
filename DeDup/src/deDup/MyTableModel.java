 ...
 * @author Frank Martyn
 * 
 *         When extending this class, Implement the "getColumnClass" method.
...
public class MyTableModel extends AbstractTableModel {
	...
	synchronized public Object getValueAt(int row, int column) {
		if ((row < 0) || (column < 0) || (row > rows) || (column > columnCount - 1)) {
			String msg = String.format("Invalid row/column setting. [Rows = %d] row = %d, Column = %d ",rows, row, column);
	...
	// String msg = String.format("[getColumnClass] Invalid column. columnIndex =
	// %d, max Column = %d",
	...
	//
	// private static final String[] headers = new String[] { COLUMN_0, COLUMN_1,
